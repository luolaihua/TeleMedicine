...
{
  ui->setupUi(this);
  // ui->photoLabel->setPixmap (QPixmap(":/b.jpg"));
  // initMainWindow();
  // initData ();
}
...
MainWindow::initMainWindow()
{
  ...
  cvtColor(
    ctImg, ctImg, COLOR_BGR2RGB); //转换色彩空间，opencv中的是BGR，Qt中的是RGB
  myCtImg = ctImg;
  myProCtImg = ctImg;
  Mat ctGrayImg;
  ...
  myCtQImage =
    QImage(ctImg.data, ctImg.cols, ctImg.rows, QImage::Format_RGB888);
...
MainWindow::connectAll()
{
  ...
  connect(
    ui->actionparamSet, &QAction::triggered, this, &MainWindow::slot_getParams);
  connect(
    ui->actionresetCT, &QAction::triggered, this, &MainWindow::slot_resetCT);
  //旋转缩放，对比度亮度调节
  ...
  /// CT相片处理中的动作
  connect(
    ui->actiongray, &QAction::triggered, this, &MainWindow::slot_img2Gray);
  //高斯滤波
  connect(ui->actionguassainBlur,
          &QAction::triggered,
          this,
          &MainWindow::slot_guassianBlur);
  connect(ui->actionbitlateBlur,
          &QAction::triggered,
          this,
          &MainWindow::slot_bilateBlur);
  //    connect(ui->actionclearNoise, &QAction::triggered, this,
  //    &MainWindow::slot_clearNoise);
  //阈值动作
  ...
  //细胞计数动作
  connect(
    ui->actionimportCell, &QAction::triggered, this, [=]() { ctImgRead(); });
  connect(ui->actionstartCount,
          &QAction::triggered,
          this,
          &MainWindow::slot_cellCount);
...
void
MainWindow::ctImgBlur(QString type, int value)
{
  ...
  enum QImage::Format m =
    (imgDst.channels() > 1) ? QImage::Format_RGB888 : QImage::Format_Grayscale8;
 ...
 */
void
MainWindow::ctImgRead()
{
  QString ctImgName = QFileDialog::getOpenFileName(
    this, "载入CT相片", ".", "Image File (*.png *.jpg *.jepg *.bmp)");
  ...
  myCtGrayImg = ctGrayImg;
  myCtQImage =
    QImage(ctRgbImg.data, ctRgbImg.cols, ctRgbImg.rows, QImage::Format_RGB888);
  ctImgShow();
}

void
MainWindow::ctImgProc()
{
  //    QTime time;
  //    time.start ();
  ...
  // (this,tr("完毕"),tr("子宫内壁见椭球形阴影，疑似子宫肌瘤"));
}
 ...
 */
void
MainWindow::ctImgSave()
{
  QString fileName = QFileDialog::getSaveFileName(
    this, tr("Sava CT"), "untitled.jpg", tr("Images (*.png *.bmp *.jpg)"));
  qDebug() << fileName;
  if (!fileName.isEmpty()) {
...
MainWindow::ctImgHoughCircles()
{
  ...
  HoughCircles(
    ctGrayImg, h_circles, HOUGH_GRADIENT, dp, minDist, param1, param2);
  //    int processValue = 45;
  ...
  for (size_t i = 0; i < h_circles.size(); i++) {
    ...
    // circle(ctColorImg, Point(h_circles[i][0], h_circles[i][1]),
    // h_circles[i][2], Scalar(0, 0, 255), 2);
  ...
  myCtQImage = QImage(
    myProCtImg.data, myProCtImg.cols, myProCtImg.rows, QImage::Format_RGB888);
}

void
MainWindow::ctImgPro_light(float contrat, int brightness)
{
  ...
  myCtQImage =
    QImage(imgDst.data, imgDst.cols, imgDst.rows, QImage::Format_RGB888);
  ctImgShow();
}

void
MainWindow::ctImgPro_scale(float angle, float scale)
{
...
void
MainWindow::slot_resetCT()
{
  // imshow ("",myCtImg);
  myProCtImg = myCtImg;
  myCtQImage =
    QImage(myCtImg.data, myCtImg.cols, myCtImg.rows, QImage::Format_RGB888);
  ctImgShow();
}
...
MainWindow::on_horizontalSlider_valueChanged(int value)
{
  ...
  if (text == "亮度") {
    ...
    ctImgPro_scale((value / 100.0) * 720.0 - 360,
                   ui->verticalSlider->value() / 30.0);
  } else {
...
MainWindow::on_horizontalSlider_sliderMoved(int position)
{
  ...
  if (text == "亮度") {
    ...
    ctImgPro_scale((position / 100.0) * 720.0 - 360,
                   ui->verticalSlider->value() / 30.0);
  } else {
...
MainWindow::on_verticalSlider_valueChanged(int value)
{
  if (ui->label_horizon->text() == "亮度") {
    ...
    ctImgPro_scale((ui->horizontalSlider->value() / 100.0) * 720.0 - 360,
                   value / 30.0);
  }
}
//垂直方向是缩放和对比度
void
MainWindow::on_verticalSlider_sliderMoved(int position)
{
  if (ui->label_horizon->text() == "亮度") {
 ...
 */
void
MainWindow::slot_img2Gray()
{
  // imshow ("",myCtGrayImg);
  myCtQImage = QImage(myCtGrayImg.data,
                      myCtGrayImg.cols,
                      myCtGrayImg.rows,
...
void
MainWindow::slot_threshold(int type)
{
  threshold_type = type;
  Mat proImg = myProCtImg;
  ui->label_horizon->show();
...
//填充
void
fillHole(const Mat srcBw, Mat& dstBw)
{
  Size m_Size = srcBw.size();
  Mat Temp =
    Mat::zeros(m_Size.height + 2, m_Size.width + 2, srcBw.type()); //延展图像
...
MainWindow::slot_cellCount()
{
  ...
  for (int i = 0; i < contours.size(); i++) {
     ...
     */
    boundRect[i] =
      boundingRect(Mat(contours_poly[i])); //计算并返回包围轮廓点集的最小矩形
    /**
     * 寻找包裹轮廓的最小圆：minEnclosingCircle 函数
     */
    minEnclosingCircle(contours_poly[i],
                       center[i],
                       radius[i]); //计算并返回包围轮廓点集的最小圆形及其半径
  ...
  myCtQImage =
    QImage(srcImg.data, srcImg.cols, srcImg.rows, QImage::Format_RGB888);
  ctImgShow();
  QMessageBox::information(
    this,
    "细胞计数",
    QString("计算成功，当前图片中细胞个数为%1").arg(contours.size()));
...
void
MainWindow::on_saveHistoryBtn_clicked()
{
...
void
MainWindow::on_basicTableView_clicked(const QModelIndex& index)
{
  onTableSelectChange(1);
}

void
MainWindow::on_tabWidget_tabBarClicked(int index)
{
  //填写病历
  if (index == 1) {
    QModelIndex index;
...
void
MainWindow::onTableSelectChange(int row)
{
  int r = 1; //默认显示第一行
  //获取当前索引
  if (row != 0)
